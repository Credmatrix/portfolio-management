...
export async function GET(request: NextRequest) {
    try {
        ...
        // Build enhanced query with business intelligence
        let query = supabase
            .from('deep_research_reports')
            .select(`
                *,
        ...
        const autoGenerated = searchParams.get('auto_generated')
        const reportType = searchParams.get('report_type')

        if (autoGenerated) {
            query = query.eq('auto_generated', autoGenerated === 'true')
        }
        ...
        // Enhance reports with business intelligence metadata
        const enhancedReports = reports?.map(report => ({
            ...report,
            company_info: {
                name: report.document_processing_requests.company_name,
                cin: report.document_processing_requests.cin,
                pan: report.document_processing_requests.pan,
                industry: report.document_processing_requests.industry
            },
            // Extract business intelligence from findings_summary
            risk_score: (report.findings_summary as any).risk_score || null,
            credit_recommendation: (report.findings_summary as any).credit_recommendation || null,
            data_quality_score: (report.findings_summary as any).data_quality_score || null,
            key_risk_factors: (report.findings_summary as any).key_risk_factors || [],
            mitigating_factors: (report.findings_summary as any).mitigating_factors || []
        })) || []
